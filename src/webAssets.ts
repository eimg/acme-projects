/**
 * Web UI delivery for the Acme Projects server.
 *
 * Built mode serves the Vite output from dist. Source mode (ACME_PROJECTS_DEV=1, set by
 * `npm run dev`) mounts Vite as middleware instead, so editing web/ takes effect over
 * HMR and `acme-projects serve` never needs a prior build.
 */
...
export function webFromSource(): boolean {
  return process.env.ACME_PROJECTS_DEV === "1" && existsSync(sourceIndex);
}
...
function viteDevServer(): Promise<ViteDevServer> {
  ...
  // standalone Acme Projects, which would make an embedded Vite proxy back to its own host.

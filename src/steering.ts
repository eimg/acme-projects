...
export async function probeSteeringIntegration(
  ...
  if (!config.baseUrl) {
    return {
      ...
      status: "unconfigured",
      detail: config.source === "stored" ? "Steering notifications are disabled in Acme Projects." : "No Steering URL is configured.",
      checkedAt,
  ...
  try {
    ...
    const credentialResponse = await fetchFn(`${url}/api/notifications/check`, {
      ...
      body: JSON.stringify({ product: "acme-projects" }),
    ...
    return {
      ...
      detail: online
        ? "Acme Steering is reachable and accepts Projects notifications."
        : `Acme Steering is reachable, but the Projects notification credential was rejected (${credentialResponse.status}).`,
      checkedAt,
      credentialConfigured,
      credentialWillBeSent,
      startupConfigured,
